^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Changelog for package ultron_description
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
* Major refactor for indigo release:
  ...
  * moved plugins requiring gazebo dependencies to ultron_gazebo (imu, gps, lidar, ros_control)
...
* ultron_description moved up to repository root.
